...
1. **Authoring** — Usually this step is an iterative approach. You can imagine it like you are in a development phase of your project. What you have to do is code execuation or more specifically:
    ...
    Note that it is best practice to have a version control service such as [GitHub](https://github.com/) to keep track of your code by the beginning of your project. In addition, you should add a `.gitignore` file containing both `.quarto` and `_site` if you want to use *Quarto* to render and publish those contents.
...
2. **Rendering** — This process is about generating a report-like document for each executable files. A fully re-rendering is required as to have all changes reflected before publishing.
    ...
    If you also want to execute a report on-the-fly. Use `--execute` flag.
    ...
    Anyway, in ML/DL projects, most of the time, you do not want to re-execute the code as it has already done by other cloud services for training the models. So, the `--execute` flag will not be used and `_freeze` directory will never be created.
    
    Then What happens if we do not have the computation results in `_freeze`? — My understanding is a file type like `.ipynb`, which is the most common file type for ML/DL projects, could have a cache within the files themselves. Therefore, they can still be rendered into `_site` and `.quarto` and when you publish your project even `_site` and `.quarto` are ignored by a `.gitignore` file, Under the hood, *Quarto* is able to re-render from those caches and publish on the hosting service without depending on `_freeze` or `_site` and `.quarto`.

    In sum, we do not need to pay attention to a `_freeze` directory as we still have rendered files in `_site` and `.quarto` (or even have our documents published successfully). But, we can not ignore setting a `freeze` option to `auto` in `_quarto.yml` because it is important for efficint-sake that we are re-rendering only if source code changes.
...
3. **Publishing** — This is another iterative step. It may required you to deploy your websites or documents several times before reaching the desired output. In my workflow, I will use [GitHub Pages](https://pages.github.com/) as a hosting service.
    ...
    If you are not using [Render to `docs`](https://quarto.org/docs/publishing/github-pages.html#render-to-docs) method to publish your website. You will need to publish manually using the following command:
    ...
    Note that you will need to do a one-time setup by following these two steps:
...
All the steps are repeating as the development continues. We are also updating the project as a **release** while the source code changes along the way.

:::{.callout-tip}
You may notice that the rendering step is kind of optional if we've already have the latest version of our documents in `_site` and `.quarto`, which can come from `quarto preview`. This allows us to skip this step if needed. However, it is recommended that we need to do `quarto render` to make sure we have the recent rendered documents. That's why it is listed as the main step in the workflow.
:::
...
Throughout the project development, there are changes made along the way and it is the best practice to not only log those changes but also document them. The additional tool that I use is called [git-cliff](https://github.com/orhun/git-cliff). Note that In most changelog generator, it is required to `tag` your commits to be able to document date changes. So, the workflow to release a documentation website with version history is as followed:
...
:::{.callout-tip}
you can use for example, `git tag -a v0.0.1 -m "version 0.0.1 - soft launch"` to tag your code after a few commits. Also, you can save your development to a remote repository using `git push origin main` or release your code to the public via `git push origin v0.0.1` or push all tags using `git push origin --tags`. For the most straightforward way to publish to *GitHub Pages* immediately after everything is setup, use `git push origin main --tags`.
:::

:::{.callout-warning}
If you are using *GitHub Actions* to automatically publish your documentation and you consider using `git tag` to release your documentation, it is better to only run your CI/CD pipeline when there is a new release on your documentation. Therefore, you may need to develop in let say another branch such as `dev`. After you complete with your development, you `merge` it into `main` and `tag` your documentation for a release. This way, it is more organized and help reduce unreleased changes to your documentation changelog.
:::
